  ...
  module Analytics
    module Persistence
      class MongoStore < Store
        ...
        def[](document_id)
          collection.find({ :_id => format_record_id(document_id)}).to_a.pop
        end
        ...
        def update_facts_record(record_id, data)
          query = { :_id => format_record_id(record_id) }
          update = { "$set" => data }
        ...
        def format_record_id(record_id)
          raise "record_id cannot be nil" unless record_id
          record_id.kind_of?(BSON::ObjectId) ? record_id : BSON::ObjectId(record_id.to_s)
        end
